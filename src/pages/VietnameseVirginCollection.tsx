...
export default function VietnameseVirginCollection() {
  const { products, loading } = useCmsProducts({ collection: "vietnamese-virgin", limit: 50 });

  return (
    <PageLayout>
      <section className="relative -mx-4 md:-mx-8 lg:-mx-16 -mt-8 md:-mt-12 mb-12">
        <div className="relative h-[40vh] md:h-[50vh] overflow-hidden">
          <img src={vietnameseImage} alt="Vietnamese Virgin Collection" className="w-full h-full object-cover" />
          ...
          <div className="absolute inset-0 flex items-end justify-center pb-12">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center">
              ...
              <h1 className="text-4xl md:text-6xl font-serif tracking-wider text-foreground">VIETNAMESE VIRGIN</h1>
              <p className="text-muted-foreground mt-4 max-w-xl mx-auto px-4">Discover the silky smoothness of Vietnamese virgin hair.</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="mt-16 bg-card rounded-2xl p-8 md:p-12">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-muted-foreground leading-relaxed">Our Vietnamese Virgin hair collection showcases the finest quality hair from Vietnam. Known for its naturally straight texture and silky feel.</p>
        </div>
      </section>
    </PageLayout>
  );
