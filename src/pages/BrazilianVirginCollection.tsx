    ...
    <PageLayout>
      <section className="relative -mx-4 md:-mx-8 lg:-mx-16 -mt-8 md:-mt-12 mb-12">
        <div className="relative h-[40vh] md:h-[50vh] overflow-hidden">
          <img src={brazilianImage} alt="Brazilian Virgin Collection" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent" />
          <div className="absolute inset-0 flex items-end justify-center pb-12">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center">
              <p className="text-sm uppercase tracking-widest text-muted-foreground mb-2">Premium Collection</p>
              <h1 className="text-4xl md:text-6xl font-serif tracking-wider text-foreground">BRAZILIAN VIRGIN</h1>
              <p className="text-muted-foreground mt-4 max-w-xl mx-auto px-4">Experience the luxury of 100% unprocessed Brazilian virgin hair.</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="mt-16 bg-card rounded-2xl p-8 md:p-12">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-muted-foreground leading-relaxed">Our Brazilian Virgin hair collection features 100% unprocessed human hair sourced from Brazil. This hair type is renowned for its natural body, versatility, and ability to blend seamlessly with various hair textures.</p>
        </div>
      </section>
    </PageLayout>
  );
